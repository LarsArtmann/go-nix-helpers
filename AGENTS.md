...
- **Module options (30 total)** — go-standard now supports: `enableCheck`, `enableOverlay`, `buildFlags`, `version`, `enableGolangciLint`, `enableGofumpt`, `enableGoimports`, `enableCompletions`, `packages` (monorepo), `validatePrivateDeps`, `privateDepPattern`, `publicDeps`. All with sensible defaults, tested by `test-module.nix`.
