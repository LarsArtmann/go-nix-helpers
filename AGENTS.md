...
- **Module options (31 total)** — go-standard now supports: `enableCheck`, `enableOverlay`, `buildFlags`, `version`, `enableGolangciLint`, `enableGofumpt`, `enableGoimports`, `enableNixfmt`, `enableCompletions`, `packages` (monorepo), `validatePrivateDeps`, `privateDepPattern`, `publicDeps`. All with sensible defaults, tested by `test-module.nix`.
