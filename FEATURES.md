...
| Module-level tests (options, types, outputs)                   | 🟢 `FULLY_FUNCTIONAL`     | `test-module.nix` with 123 assertions; behavioral tests for buildFlags, ldflags, proxyVendor, GOPRIVATE, requireDeps forwarding, extraBuildAttrs concatenation (6 attrs), enableCompletions negative path, treefmt config inspection, shfmt toggle, enableTempl=false alone, monorepo version propagation, goPkgAttr auto-default resolution |
...
| Module test suite (`test-module.nix`)                    | 🟢 `FULLY_FUNCTIONAL`     | 123 assertions; behavioral tests for buildFlags/ldflags/proxyVendor/GOPRIVATE/requireDeps forwarding/extraBuildAttrs (6 attrs)/enableCompletions negative/treefmt config/shfmt toggle/enableTempl=false/monorepo version/goPkgAttr auto |
| Pure function property tests (`test-pure-functions.nix`) | 🟢 `FULLY_FUNCTIONAL`     | 50 assertions: idempotence, no-`/vN`-in-output, determinism, no-slash, edge cases, `newestGoAttrName` numeric compare, `goBaseFrom` resolution (pin/auto/fallback/override/tarball) — wired as `checks.pureFunctions`                                                          |
