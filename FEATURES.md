...
| Module test suite (`test-module.nix`)                    | 🟢 `FULLY_FUNCTIONAL`     | 121 assertions; behavioral tests for buildFlags/ldflags/proxyVendor/GOPRIVATE/extraBuildAttrs (6 attrs)/enableCompletions negative/treefmt config/shfmt toggle/enableTempl=false/monorepo version/goPkgAttr auto |
| Pure function property tests (`test-pure-functions.nix`) | 🟢 `FULLY_FUNCTIONAL`     | 41 assertions: idempotence, no-`/vN`-in-output, determinism, no-slash, edge cases, `newestGoAttrName` numeric compare — wired as `checks.pureFunctions`                                                          |
