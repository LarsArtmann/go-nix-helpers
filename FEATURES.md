...
| Configurable `systems` option (no longer hardcoded)            | 🟢 `FULLY_FUNCTIONAL`     | `go-standard.systems` option; defaults to `nix-systems/default` minus `x86_64-darwin` (nixpkgs 26.11 dropped it)                                                                     |
...
| `enableTestCheck` option (hermetic `checks.test`)              | 🟢 `FULLY_FUNCTIONAL`     | Forces `go test` in CI even when `enableCheck = false`; defaults to `false`                                                                                                                                      |
| `goPkgAttr` auto default (newest `go_1_XX`)                    | 🟢 `FULLY_FUNCTIONAL`     | `null` resolves via `newestGoAttrName` (numeric compare, `pkgs.go` fallback); explicit pins win                                                                                                                  |
| `goPkgOverride` / `goTarballVersion`+`goTarballHash`           | 🟢 `FULLY_FUNCTIONAL`     | Custom toolchain composition; go.dev source tarball build when nixpkgs' newest branch is below the go.mod floor                                                                                                  |
| `lintAsCheck` option (hermetic `checks.lint`)                  | 🟢 `FULLY_FUNCTIONAL`     | Gated on `enableGolangciLint`; defaults to `false`                                                                                                                                                               |
| `requireDeps` option (manual require injection)                | 🟢 `FULLY_FUNCTIONAL`     | Deduped against existing go.mod requires; unblocks sub-modules with explicit version pins                                                                                                                        |
| `templ-committed` eval check                                   | 🟢 `FULLY_FUNCTIONAL`     | Throws at `nix flake check` when a `.templ` file lacks its committed `*_templ.go` sibling; context-free path arithmetic (works under `--no-build`)                                                                 |
...
| Module-level tests (options, types, outputs)                   | 🟢 `FULLY_FUNCTIONAL`     | `test-module.nix` with 139 assertions; behavioral tests for buildFlags, ldflags, proxyVendor, GOPRIVATE, requireDeps forwarding, extraBuildAttrs concatenation (6 attrs), enableCompletions negative path, treefmt config inspection, shfmt toggle, enableTempl=false alone, monorepo version propagation, goPkgAttr auto-default resolution, goExperiment/cgoEnabled propagation, completionStyle, go.mod floor check, stale-pin warning path |
...
| Feature                                              | Status                | Notes                                                                                   |
| ---------------------------------------------------- | --------------------- | --------------------------------------------------------------------------------------- |
| `scripts/dashboard.sh` — flake-status overview       | 🟢 `FULLY_FUNCTIONAL` | Supports `--check`, `--json`; GO_LATEST auto-derived from `lib.newestGoAttr` (no manual bump site) |
| `scripts/nix-lint.sh` — flake.nix linting            | 🟢 `FULLY_FUNCTIONAL` | Supports `--fix`, `--check`; go_1_XX patterns generalized                                                             |
| `scripts/nix-health.sh` — local build pre-flight    | 🟢 `FULLY_FUNCTIONAL` | Canary for the 2026-09-25 silent host break: binfmt vs nix.conf, daemon age, auto-GC exposure, store headroom            |
| `scripts/check-docs-annotations.sh` — docs gates    | 🟢 `FULLY_FUNCTIONAL` | Vendored from docs-health skill (attribution kept); archived completeness + table row uniformity; guard-the-guard verified red+green |
...
| Integration test suite (`test.nix`)                      | 🟢 `FULLY_FUNCTIONAL`     | 9 scenarios: auto-discovery, explicit-only, validation, publicDeps exclusion, requireDeps dedup, multi-deps monorepo, publicDeps /v2 versioned paths, in-tree replace stripping, pseudo-version normalization    |
| Module test suite (`test-module.nix`)                    | 🟢 `FULLY_FUNCTIONAL`     | 139 assertions; behavioral tests for buildFlags/ldflags/proxyVendor/GOPRIVATE/requireDeps forwarding/extraBuildAttrs (6 attrs)/enableCompletions negative/treefmt config/shfmt toggle/enableTempl=false/monorepo version/goPkgAttr auto/goExperiment+cgoEnabled/completionStyle/go.mod floor/stale pin |
| Pure function property tests (`test-pure-functions.nix`) | 🟢 `FULLY_FUNCTIONAL`     | 61 checks: idempotence, no-`/vN`-in-output, determinism, no-slash, edge cases, `newestGoAttrName` numeric compare, `staleGoAttrName`, `goModFloorMessage` content, `goBaseFrom` resolution (pin/auto/fallback/override/tarball) — wired as `checks.pureFunctions` |
| Docs-annotation gates (`checks.docsAnnotations`)        | 🟢 `FULLY_FUNCTIONAL`     | Gate 1: every archived status report carries a strikethrough resolution; gate 2: no table mixes struck/unstruck rows (vendored `check-rows.py`). Guard-the-guard verified red on planted defects |
...
| Template eval check (`checks.templateEval`)              | 🟢 `FULLY_FUNCTIONAL`     | Evaluates the go-standard template's `outputs` fn with mock inputs; catches unbound-variable bugs                                                                                                |
| Templ fixture guard (`checks.templFixtureGuard`)         | 🟢 `FULLY_FUNCTIONAL`     | Fails the flake check when generated `*_templ.go` files become tracked under the missing-generated fixture; turns the daemon's re-add failure mode into a named error. Verified red+green |
