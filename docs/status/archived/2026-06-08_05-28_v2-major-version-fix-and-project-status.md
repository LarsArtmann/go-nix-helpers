...
### ~~1. Downstream consumers — Need `vendorHash` updates~~ done — consumer-side — repos long since rebuilt (PMA later migrated to go-standard `9b47684`)
...
### ~~2. `projects-management-automation` — Manual `postPatch` workarounds~~ done — done — PMA migrated to go-standard (`9b47684`)
...
### ~~2. No automated tests~~ done — shipped — test.nix (9 scenarios) + moduleTest (121) + pureFunctions (41)

No test suite exists. Verification is manual via `nix eval` and downstream consumer builds.

### ~~3. No CHANGELOG.md~~ done — shipped — CHANGELOG.md exists
...
### ~~5. `report/` directory is empty~~ done — moot — directory long gone
...
### ~~1. Test infrastructure~~ done — shipped — full checks suite
...
### ~~2. Reduce downstream manual work~~ done — shipped — go-standard module + fleet migrations
...
### ~~3. Error handling for ambiguous paths~~ **Won't implement — no incidents — dropped.**
...
### ~~4. `subModuleVersionNormalize` is fragile~~ done — hardened — `[.]` style after the 2026-09-07 escape-semantics probe
...
### ~~5. Template auto-generation~~ **Won't implement — checks.templateEval guards the template instead.**
