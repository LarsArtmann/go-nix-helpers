...
| - | --------------------------------------------------------------- | --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ~~1~~ | ~~**Skill cross-references**~~ | ~~All three skills mention go-standard and link to the repo~~ | ~~Skills don't mention the _full option list_ (enableTempl, deps, subModules, ldflags, etc.) — they'd need to visit the repo README for that~~ | **Won't implement — SKILLS-repo scope — addressed in the 2026-08-12 skill audit sessions.**
| ~~2~~ | ~~**nix-private-go-repos Option A example**~~ | ~~Shows go-standard with `deps`~~ | ~~Doesn't show how to override `validatePrivateDeps` or `privateDepPattern` via go-standard options~~ | **Won't implement — SKILLS-repo scope — addressed in the 2026-08-12 skill audit sessions.**
| ~~3~~ | ~~**Skill sync to `.agents/skills/` and `.config/crush/skills/`**~~ | ~~The copies exist~~ | ~~I have no idea how they get synced — unclear if manual, scripted, or automatic. If automatic, the updated SKILLS repo changes may or may not propagate.~~ | **Won't implement — SKILLS-repo scope — symlink conversion tracked in the SKILLS repo.**
...
| - | ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ | -------------- |
| ~~1~~ | ~~**End-to-end consumer test**~~ | ~~The composite module has never been consumed by a real project. Structural eval passes but real consumption is unproven.~~ | ~~Report #1~~ | done — moduleTest + templateEval + 10 real consumers; build E2E in TODO_LIST T3
| ~~2~~ | ~~**Fix `defaultSystems` to use `import inputs.systems`**~~ | ~~go-nix-helpers already declares `systems.url` — should use it instead of hardcoding~~ | ~~Report #1~~ | done — shipped — systems option (2026-07-24)
| ~~3~~ | ~~**Consumer migration plan**~~ | ~~7+ projects still on 5-input manual pattern~~ | ~~Report #1~~ | done — done — fleet audit + Tier A migrations (2026-08-10)
| ~~4~~ | ~~**Double-import conflict test**~~ | ~~Consumer with both treefmt-nix input AND composite module — untested~~ | ~~Report #1~~ | done — moot — adopters declare no treefmt-nix input
| ~~5~~ | ~~**Automated test for module outputs**~~ | ~~test.nix only tests mkPreparedSource, not the module itself~~ | ~~Report #1~~ | done — shipped — checks.moduleTest (121 assertions)
...
| - | ------------------------------------------------ | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| ~~1~~ | ~~**Auto-commit mechanism STILL not investigated**~~ | ~~CRITICAL~~ | ~~First report flagged 4 mystery commits. Since then, 3 MORE mystery commits appeared in SKILLS repo (`5a786ec`, `3a48f77`, `ac7336d`) and 1 in go-nix-helpers (`503bc40`). I never ran `git commit` in either repo. **9 total mystery commits this session.** I did not investigate this despite flagging it as HIGH severity in report #1.~~ | done — identified — buildflow file-watcher (2026-07-24)
| ~~2~~ | ~~**`defaultSystems` still hardcoded**~~ | ~~MEDIUM~~ | ~~Report #1 identified this as architecturally worse than `import inputs.systems`. Still not fixed. Two lines of code.~~ | done — shipped — systems option
| ~~3~~ | ~~**`test-result` symlink still in git**~~ | ~~LOW~~ | ~~Report #1 flagged this. Still tracked. `.gitignore` still doesn't cover it. **Resolved (2026-07-24):** removed from git tracking and `.gitignore`d.~~ | done — resolved 2026-07-24
| ~~4~~ | ~~**`flake.lib.mkGoFlake` still exported**~~ | ~~LOW~~ | ~~Deprecated module still exported in flake.nix line 38. Split brain with go-standard.~~ | done — deprecation trace shipped; deletion gated v0.1.0
| ~~5~~ | ~~**`go-flake-parts` template not updated**~~ | ~~MEDIUM~~ | ~~Still shows 5-input manual pattern with REPLACE_ME placeholders. Now misleading since go-standard template exists and is recommended. Should be deleted or marked as legacy.~~ | done — deprecated with banner
| ~~6~~ | ~~**Skills don't list go-standard options**~~ | ~~MEDIUM~~ | ~~I added go-standard as recommended but didn't include even a summary of available options. A future session using the skill would have to break flow and visit the repo README to know what `enableTempl`, `deps`, `ldflags` etc. are available.~~ | **Won't implement — SKILLS-repo scope — addressed in the 2026-08-12 audits.**
...
### ~~1. What is creating commits without me running `git commit`?~~ done — buildflow file-watcher — identified 2026-07-24
...
### ~~3. Should I fix the open issues from report #1 now, or do you want to migrate a consumer first?~~ done — module tests and real consumers shipped since
