...
| ~~14~~ | ~~Add test for `stripLocalReplaces = false`~~ | ~~Option exists but disabled state is untested~~ | ~~15min~~ **Won't implement — dormant — dropped (reopen on demand).** |
...
| ~~18~~ | ~~Add test for monorepo + extraBuildAttrs concatenation~~ | ~~Ensures merge logic works in multi-package mode~~ | ~~30min~~ **Won't implement — dormant — dropped (reopen on demand).** |
| ~~19~~ | ~~Add test for `shellExtraBuildInputs` propagation~~ | ~~devShell extras may be under-tested~~ | ~~20min~~ |
| ~~20~~ | ~~Add test for `extraMeta` propagation to all packages in monorepo~~ | ~~Ensures meta flows to each package~~ | ~~20min~~ **Won't implement — dormant — dropped (reopen on demand).** |
...
| ~~48~~ | ~~Add a consumer-facing changelog (breaking changes only)~~ | ~~Helps downstream consumers decide when to update~~ | ~~30min~~ **Won't implement — dormant — dropped (reopen on demand).** |
