...
2. ~~**Two-phase sub-module handling increases cognitive complexity.** Explicit `subModules` are handled at eval time (Nix), auto-discovered at build time (shell). The dedup logic between them is now a shell `grep` check rather than `lib.unique`. This works but is harder to reason about than the old unified list. The tradeoff was necessary to fix `--no-build`.~~ **Won't implement — accepted tradeoff — the two-phase design is the fix for --no-build.**

3. ~~**The shell-based module path extraction (`awk '/^module /{print $2; exit}'`) duplicates logic** that was previously in `readModulePath` (Nix). Both do the same thing but in different languages. If the go.mod format ever changes (unlikely), both need updating.~~ **Won't implement — accepted — the awk one-liner replaced the deleted eval-time function.**

4. ~~**No build-time test for the dedup logic.** The existing `verify` check tests that explicit and auto-discovered entries are both present, but doesn't test the case where an explicit `subModules` entry DUPLICATES an auto-discovered one. The old `lib.unique` guaranteed dedup; the new `grep -qF` check should too, but there's no dedicated test.~~ **Won't implement — dormant — dropped.**

### Process Critique

5. ~~**I should have inspected individual test assertions**, not just exit code 0. The `verify` check produces a directory with `result.txt` saying "all success-path tests passed" — but I didn't see the individual PASS/FAIL lines because they're consumed by the derivation build. I verified the go.mod output manually for two cases, which partially compensates.~~ **Won't implement — process lesson — recorded.**

6. ~~**I didn't check if `readModulePath` was referenced elsewhere** before removing it. It was a `let`-bound function local to `mkPreparedSource.nix`, so this was safe — but I should have verified with a grep before deleting.~~ **Won't implement — process lesson — recorded.**

7. ~~**I initially treated the paste as 6 independent issues** when actually 4 of them were already fixed. I spent time reading files and checking formatting before realizing the working tree was clean. A `git status` + `git log --oneline -1` at the very start would have immediately shown the state.~~ **Won't implement — process lesson — recorded.**

8. ~~**The `test-module.nix` `{self}` mismatch** was introduced by commit `c0a8290` (which removed `self` from the parameter list but didn't update `flake.nix`). This means CI was already broken before this session. I should have caught this faster by running `nix flake check --no-build` immediately after seeing the first error.~~ **Won't implement — process lesson — recorded.**

### Missing Coverage

9. ~~**No test for the `autoSubModules = false` + `subModules` + auto-discovery interaction.** When auto-discovery is disabled, the script is skipped entirely (`lib.optionalString autoSubModules`). The `explicitOnly` test covers this case, but only with a single dep and single sub-module.~~ **Won't implement — dormant — the explicitOnly scenario covers the disabled path.**

10. ~~**No test for deeply nested excluded directories.** The shell `case` pattern `*/example/*` should match at any depth, but there's no test for `_local_deps/X/a/b/example/c/go.mod`.~~ **Won't implement — dormant — dropped.**
...
1. ~~Add a test case where explicit `subModules` entry DUPLICATES an auto-discovered one — verify the dedup works at build time~~ **Won't implement — dormant — dropped.**
...
4. ~~Run `nix flake check --no-build` in CI to confirm it passes in a clean environment (not just locally with cached derivations)~~ done — CI builds the checks in the integration-tests job
5. ~~Add a test for excluded directories at depth > 1 (e.g. `_local_deps/X/a/example/b/go.mod`)~~ **Won't implement — dormant — dropped.**
...
10. ~~Consider adding `shellcheck` to the CI for the generated shell script in `autoDiscoverScript`~~ **Won't implement — dormant — dropped.**
11. ~~Add `autoDiscoverScript` output to the `verify` check's diagnostic output (for debugging)~~ **Won't implement — dormant — dropped.**
12. ~~Document the eval-time vs build-time boundary in `docs/flake-patterns.md`~~ **Won't implement — dormant — dropped.**
...
14. ~~Add a property test: "discovered replaces never contain `/vN/` in the localDir when the physical directory doesn't have it"~~ **Won't implement — dormant — dropped.**

### Lower Priority (P2 — nice to have)

15. ~~Explore whether `builtins.pathExists` could replace `-mindepth 3` for a more robust root-skip~~ **Won't implement — consider-stage — dropped; the mindepth assumption is documented instead (T18).**
16. ~~Add `--trace-verbose` support to `autoDiscoverScript` for debugging consumer builds~~ **Won't implement — dormant — dropped.**
17. ~~Consider adding a `failOnNoDiscovery` option for consumers who expect auto-discovery to find at least one sub-module~~ **Won't implement — dormant — dropped.**
18. ~~Benchmark: does build-time discovery slow down the build noticeably vs eval-time?~~ **Won't implement — dormant — Nix already caches the derivation.**
...
20. ~~Add a `--list-discovered` debug app that shows what would be discovered without building~~ **Won't implement — dormant — dropped.**
21. ~~Consider whether `subModuleVersion` normalization should also run on main dep replaces~~ **Won't implement — dormant — dropped.**
22. ~~Review whether the `case` pattern for excluded dirs handles all edge cases (spaces, special chars)~~ done — moved to TODO_LIST T20 (same quoting class)
23. ~~Add integration test with a dep that has go.mod in the root only (no sub-modules)~~ **Won't implement — dormant — dropped.**
24. ~~Add integration test with a dep that has go.mod ONLY in subdirectories (no root go.mod)~~ **Won't implement — dormant — dropped.**
25. ~~Consider whether `find -mindepth 3` should be configurable via a parameter~~ **Won't implement — dormant — dropped.**
26. ~~Document the shell portability of `autoDiscoverScript` (bashisms: `case`, `[[ ]]`, `${var#pattern}`)~~ **Won't implement — dormant — dropped.**
27. ~~Add a test for the scenario where `_local_deps/` is empty (no deps)~~ **Won't implement — dormant — dropped.**
28. ~~Add a test for the scenario where a dep has a `vendor/` directory with go.mod files~~ **Won't implement — dormant — dropped.**
29. ~~Review whether `node_modules` exclusion is still needed (Go projects shouldn't have it)~~ **Won't implement — dormant — dropped.**
30. ~~Consider adding `dist/`, `build/`, `bin/` to default `excludeSubModuleDirs`~~ **Won't implement — dormant — dropped.**
...
38. ~~Add a test for module paths with hyphens (e.g. `github.com/go-cqrs-lite`)~~ **Won't implement — dormant — dropped.**
39. ~~Review whether the `grep -qF` dedup in the replace-merge step handles whitespace variations~~ **Won't implement — dormant — dropped.**
...
41. ~~Update `docs/architecture.d2` diagram to show the build-time discovery flow~~ **Won't implement — dormant — dropped.**
...
43. ~~Add a benchmark comparing eval-time vs build-time discovery performance~~ **Won't implement — dormant — dropped.**
...
45. ~~Review whether the `excludeSubModuleDirs` default list matches real-world LarsArtmann repos~~ **Won't implement — dormant — dropped.**
46. ~~Add a test for symlinks in dep sources (should they be followed?)~~ **Won't implement — dormant — dropped.**
47. ~~Consider adding `GOFLAGS=-mod=mod` to the build environment for go.mod modifications~~ **Won't implement — dormant — dropped.**
48. ~~Review whether `chmod -R u+w _local_deps` is still needed (it was for read-only store paths)~~ **Won't implement — dormant — dropped.**
49. ~~Consider documenting the order of operations in postPatch (copyDeps → stripLocalReplaces → postPatchExtra → normalize → discover → requires → replaces → validate)~~ **Won't implement — dormant — dropped.**
50. ~~Consider whether the `verify` check should also test `requireDedupTest`, `multiDepsTest`, `publicDepsTest`, and `versionedPublicDepsTest` individually (currently they're built but not verified by the check script)~~ **Won't implement — dormant — dropped.**

---

## g) Questions I Cannot Answer Myself

1. ~~**Should the `verify` check script test ALL 7 test derivations** (`autoDiscovery`, `explicitOnly`, `publicDepsTest`, `requireDedupTest`, `multiDepsTest`, `versionedPublicDepsTest`, `validationTest`)? Currently it only runs assertions on `autoDiscovery`, `explicitOnly`, and `publicDepsTest`. The others are built (so they must not crash) but their go.mod outputs aren't verified by the check script. Is this intentional (the build succeeding is enough) or a gap?~~ done — decided — build success is the gate; output assertions cover the core scenarios

2. ~~**Are there downstream consumers that inspect the `go.mod` output of `mkPreparedSource` during Nix evaluation?** The build-time discovery change means the replace directives for auto-discovered modules are no longer available at eval time. Any consumer that reads the go.mod at eval time (e.g. for validation or custom logic) would break. I can't determine this without access to the consumer repos.~~ done — answered — no consumer reads go.mod at eval; the build-time migration was the fix for that class

3. ~~**Should the eval-time `discoverSubModules` / `readModulePath` functions be kept in `pure-functions.nix` as testable utilities**, even though they're no longer used by `mkPreparedSource`? They were never there (they were `let`-bound in mkPreparedSource), but the concept of testing module-path extraction in isolation might be valuable for the shell-side `awk` equivalent.~~ done — decided — removed; the shell-side awk owns extraction
