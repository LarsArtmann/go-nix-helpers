...
| B4  | flake.nix: add `templ-fixture-guard` check (`git ls-files | grep mock-templ-missing-generated.*_templ.go` → fail) | 12m |
...
| D7  | publicDeps: escape ERE metachars (`sed 's/[.[\*^$()+?{|]/\\&/g'`) before grep -vE; Test 7 still green         | 12m |
