...
let
  ...
  # Mock sources exercising the .templ / *_templ.go sibling contract.
  mkTemplSrc =
    withTempl: withGenerated:
    pkgs.runCommandLocal "mock-templ-src" { } ''
      mkdir -p $out/web
      cat > $out/go.mod <<'EOF'
      module github.com/larsartmann/mock-templ-project
      go 1.26
      EOF
      ${lib.optionalString withTempl "printf 'package web\n' > $out/web/home.templ"}
      ${lib.optionalString withGenerated "printf '// Code generated by templ; DO NOT EDIT.\npackage web\n' > $out/web/home_templ.go"}
    '';

  mkTemplChecks =
    withTempl: withGenerated:
    let
      templSelf.outPath = mkTemplSrc withTempl withGenerated;
      modEval = lib.evalModules {
        modules = [
          flakePartsStub
          ./modules/go-standard.nix
  ...
  additionalChecks = [
    ...
    (assertCheck "no .templ files: no templ-committed check emitted" (
