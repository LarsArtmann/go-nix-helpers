...
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ------ | ------ | --------------------------------------------------------------------------------------------------- |
| Make `autoGoPrivateEnv` aware of `publicDeps` — `GOPRIVATE` blanket-marks ALL LarsArtmann repos as private in devShells, including public ones listed in `publicDeps` | TODO   | High   | 1h     | `modules/go-standard.nix:473-476`; concurrent report `docs/status/2026-08-03_02-47_*.md` section E1 |
...
| Fix README troubleshooting section — still references old error text "private modules without local replace"; add `publicDeps` as remediation option | TODO   | Med    | 15min  | `README.md:273`; concurrent report `docs/status/2026-08-03_02-47_*.md` section C                              |
| Document `publicDeps` path-exact matching behavior — does not handle `/v2` versioned module paths; needs prefix matching or prominent documentation  | TODO   | Med    | 20min  | `mkPreparedSource.nix` — uses `grep -vFx` (exact match); concurrent report E2                                 |
